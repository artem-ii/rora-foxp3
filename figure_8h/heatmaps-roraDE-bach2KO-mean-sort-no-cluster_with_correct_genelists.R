...
keep <- apply(bach2exprs[2:7], 1, function(x) !any(x<0.01))
bach2exprs <- bach2exprs[keep,]
#bach2exprs[2:7] <- filterByExpr(bach2exprs[2:7])
...
roraDE.up <- filter(diff_exp_data_old, logFC >= 0.58, P.Value <= 0.05)$SYMBOL
roraDE.down <- filter(diff_exp_data_old, logFC <= -0.58, P.Value <= 0.05)$SYMBOL
